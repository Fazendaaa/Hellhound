...
Currently, in the company that I work for we have a CLI (Command Line Interface) made in [Python](https://www.python.org/) called `estat` that you can read more about it right [here](https://github.com/Fazendaaa/Succubus). As the CLI tool as many commands and features, one of them being able to set "recipes" for projects, the ideia behind {{ pkg.name }} is to have a marketplace to share thoses recipes to be used between many people and projects.
